...
export class DemoComponent{
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      ...
      if(JSON.stringify(params) != "{}"){
        ...
        this.newsDate = this.crawler[0];
        this.titleArr = this.allDate.newsTitle;
        this.newsTitle = this.titleArr[0];
  ...
  analysisResult(){
    ...
    console.log(this.id);
    this.router.navigate(['/text_result'],{queryParams: { "id": this.id,"allDate":this.allDate}});
  }
  output(item){
    if(item){
      this.result='';
